...
(function (root) {
  ...
  function montarSecao() {
    const jaTem = document.getElementById("amor");
    if (jaTem) return jaTem;

    const secaoNova = document.createElement("section");
    secaoNova.className = "amor-secao";
    secaoNova.id = "amor";
    secaoNova.setAttribute("data-barra-amor", "true");
    secaoNova.innerHTML = [
  ...
  function ligar(secao) {
    const titulo = secao.querySelector(".amor-titulo");
    if (titulo && !titulo.textContent.trim()) titulo.textContent = TITULO;
    ...
    if (!nivelEl || !pctEl || !fraseEl || !seqEl || !btn) return secao;
  ...
  function inserir() {
    ...
    if (!secao.isConnected) {
      ...
      const depoisDoHero = destino.querySelector("#inicio");
      ...
      if (depoisDoHero && depoisDoHero.parentNode) {
        depoisDoHero.parentNode.insertBefore(secao, depoisDoHero.nextSibling);
      } else if (final) destino.insertBefore(secao, final);
